...
DEFAULT_CONFIG_PATH = AUTO_UPDATER_DIR / "financial_refresh_config.json"
DEFAULT_LOG_DIR = AUTO_UPDATER_DIR / "logs"
DEFAULT_LOCK_PATH = AUTO_UPDATER_DIR / "financial_refresh.lock"
DEFAULT_DB_PATH = PROJECT_ROOT / "data" / "site.db"
DEFAULT_SMOKE_TICKERS = ["2330", "2317", "2454"]
UPDATE_SCRIPT = SCRIPTS_DIR / "update_financials.py"
...
class Logger:
    def __init__(self, log_dir: Path) -> None:
        ...
        self.log_path = log_dir / f"financial-refresh-{timestamp}.log"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the financial-refresh pipeline with retries, optional smoke tickers, "
    ...
    parser.add_argument("--update-timeout-seconds", type=int, help="Timeout for update_financials.py runs.")
    ...
    parser.add_argument("--dry-run", action="store_true", help="Pass --dry-run to update_financials.py.")
...
def resolve_settings(args: argparse.Namespace, config: dict[str, Any]) -> dict[str, Any]:
    ...
    settings: dict[str, Any] = {
        ...
        "retry_delay_seconds": int(config.get("retry_delay_seconds", 45)),
        "update_timeout_seconds": int(config.get("update_timeout_seconds", 7200)),
...
def run_preflight(python_executable: Path, logger: Logger) -> None:
    command = [
        ...
        "-c",
        "import numpy; import pandas; import yfinance; print('preflight-ok')",
    ]
...
def run_smoke_phase(
    ...
    if unresolved:
        raise SystemExit(
            "Smoke phase did not pass for: "
            + ", ".join(unresolved)
            + ". Aborting before the full-scope refresh."
        )
    return outcomes
...
def run_full_refresh(
    ...
    command = [str(python_executable), str(UPDATE_SCRIPT), *scope_args]
    result = run_command("Full financial refresh", command, settings["update_timeout_seconds"], logger)
    if result.returncode != 0:
...
def main() -> int:
    ...
    try:
        ...
        logger.section("Financial refresh pipeline")
        ...
        if skipped_count > 0 or failed_count > 0:
            logger.line("Financial refresh completed with unresolved tickers.")
            return 1

        logger.line("Financial refresh completed successfully.")
        return 0
    finally:
        if lock_acquired:
